	...
	try {
		...
		for (const match of upcomingMatchesDB) {
            ...
            if (eventViewAPIResponse === undefined) {
                logger.warn(`!!! The match with the id ${match.api_id} doesn't have an event view !!! - check whether there is the corresponding match with another ID or not`)
        ...
        logger.info(
            `The process finished at: ${finishDate.toString()}, TOTAL DURATION :', ${duration}`,
		)
