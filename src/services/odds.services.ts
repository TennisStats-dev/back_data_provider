...
export const preMatchOddsHandler = async (bet365Id: number, matchId: number): Promise<IPreOdds | null> => {
    ...
    if (bet365Id !== undefined) {
        ...
        if (eventOdss?.stats?.matching_dir !== undefined && eventOdss.odds[config.api.constants.oddsMarketsRef.winner] !== undefined && eventOdss.odds[config.api.constants.oddsMarketsRef.winner].length > 0) {
